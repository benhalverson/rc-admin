...
describe('AddProductComponent', () => {
  ...
  const mockColorsSignal = jasmine.createSpy('colors').and.returnValue(mockColors);
  const mockColorsLoadingSignal = jasmine.createSpy('colorsLoading').and.returnValue(false);  beforeEach(async () => {
    const productServiceSpy = jasmine.createSpyObj('ProductService', ['createProduct', 'getColors'], {
    ...
    await TestBed.configureTestingModule({
      imports: [AddProductComponent, HttpClientTestingModule, RouterTestingModule, ReactiveFormsModule],
      providers: [
        provideAnimations(),
        provideToastr({
          timeOut: 3000,
  ...
  describe('File Upload Handlers', () => {
    ...
    it('should handle gallery image upload', () => {
      ...
      expect(component.productForm.get('imageGallery')?.value).toContain(testUrl);
    });
    ...
    it('should add multiple gallery images', () => {
      ...
      expect(component.productForm.get('imageGallery')?.value).toEqual([url1, url2]);
    });
  });
  ...
  describe('Form Submission', () => {
    ...
    it('should submit valid form successfully', async () => {
      ...
      expect(toastrService.success).toHaveBeenCalledWith('Product added successfully!');
    });
