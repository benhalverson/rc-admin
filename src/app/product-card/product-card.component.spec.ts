...
describe('ProductCardComponent', () => {
  ...
  it('should display product information correctly', () => {
    ...
    const filamentTypeElement = fixture.debugElement.query(By.css('.bg-blue-100'));

    expect(nameElement.nativeElement.textContent.trim()).toBe('Test Product');
    expect(descriptionElement.nativeElement.textContent.trim()).toBe('This is a test product description that should be displayed in the card');
  ...
  it('should display product image with correct attributes', () => {
    ...
    expect(imageElement.nativeElement.src).toBe('https://example.com/test-image.jpg');
    expect(imageElement.nativeElement.alt).toBe('Test Product');
  });

  it('should display color information', () => {
    const colorElement = fixture.debugElement.query(By.css('.w-4.h-4.rounded-full'));
    const colorTextElement = fixture.debugElement.query(By.css('.text-xs.text-gray-500'));

    expect(colorElement.nativeElement.style.backgroundColor).toBe('rgb(255, 0, 0)');
  ...
  it('should display different filament types correctly', () => {
    fixture.componentRef.setInput('product', { ...mockProduct, filamentType: 'PETG' });
    fixture.detectChanges();
  ...
  it('should truncate long descriptions', () => {
    const longDescription = 'This is a very long product description that should be truncated to show only the first two lines and then cut off with ellipsis to maintain the card layout consistency across all product cards in the grid view';
    fixture.componentRef.setInput('product', { ...mockProduct, description: longDescription });
    fixture.detectChanges();

    const descriptionElement = fixture.debugElement.query(By.css('p'));
    expect(descriptionElement.nativeElement.classList).toContain('line-clamp-2');
  });

  it('should truncate long product names', () => {
    const longName = 'This is a very long product name that should be truncated';
    fixture.componentRef.setInput('product', { ...mockProduct, name: longName });
    fixture.detectChanges();
